...
/* ── Illustration: 3 Claude logos (white) + lock ── */
function IllustrationPlaceholder() {
  // The sparkle path is drawn in a 512x510 viewBox. We scale it down.
  const logoScale = 0.08;
  const logoW = 512 * logoScale; // ~41px
  const logoH = 510 * logoScale; // ~41px

  return (
    <svg width="260" height="150" viewBox="0 0 260 150" fill="none" xmlns="http://www.w3.org/2000/svg">
      {/* Three Claude logos in white */}
      <g transform={`translate(${45 - logoW / 2}, ${45 - logoH / 2}) scale(${logoScale})`}>
        <path d={CLAUDE_SPARKLE_PATH} fill="white" fillRule="nonzero" />
      </g>
      <g transform={`translate(${130 - logoW / 2}, ${40 - logoH / 2}) scale(${logoScale})`}>
        <path d={CLAUDE_SPARKLE_PATH} fill="white" fillRule="nonzero" />
      </g>
      <g transform={`translate(${215 - logoW / 2}, ${45 - logoH / 2}) scale(${logoScale})`}>
        <path d={CLAUDE_SPARKLE_PATH} fill="white" fillRule="nonzero" />
      </g>

      {/* Lock icon centered below */}
